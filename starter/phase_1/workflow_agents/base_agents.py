...
def retry_with_backoff(max_retries=10, initial_delay=1):
    def decorator(func):
        @wraps(func)
...
class DirectPromptAgent:
    ...
    @retry_with_backoff(max_retries=10, initial_delay=2)
...
class RAGKnowledgePromptAgent:
    ...
    def chunk_text(self, text):
        ...
        """
        # Normalize whitespace but preserve newlines for chunking
        original_text = text
        text = text.strip()
        ...
        while start < len(text):
            ...
            # Look for newline or other natural breaks within the chunk
            for sep in ["\n", ". ", "? ", "! "]:
                if sep in text[start:end]:
                    # Find the last occurrence within the chunk bounds
                    last_pos = text[start:end].rfind(sep)
                    if last_pos != -1:
